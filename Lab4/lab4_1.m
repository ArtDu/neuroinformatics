...
[X1, Y1] = arrangeEllipse(-0.25, 0.25, 0.2, 0.2, 0, 0.025);
[X2, Y2] = arrangeEllipse(0, 0, 0.7, 0.5, -pi/3, 0.025);
[X3, Y3] = arrangeEllipse(0, 0, 1, 1, 0, 0.025);
...
%1.2 Разделение точек на обучающее
...
%1.4 Обьединение соответствующего подмножества точек в обучающее и тестовое
...
%1.7 Отображение структуры сети
view(net);

%1.8/1.9 Проверка качества
...
%1.10 Классификация точек области
...
%1.12 Классификация точек области
A = net([gX(:)';gY(:)']);
n = length(gX);
A = max(0, min(1, A));
A = round(A * 10) * 0.1;
